    ...
    public class MPathConstraint : MonoBehaviour, IAnimatorListener
    {
       ...
       // public Vector3 PathDirection { get; private set; }
        ...
        public void MoveOnPath(float weight)
        {
            ...
            MDebug.Draw_Arrow(animal.Position, PathStartEndDirection, Color.cyan); //Real Path Direction From Start to End
            ...
            //Clean UP 
            // if (Path.IgnoreVertical || !Path.usePathRotation)
            //{
            //    PathDirection = Vector3.ProjectOnPlane(PathDirection, animal.Up);
            //}
            ...
            if (Dot > 0) //Make the Animal able to turn 180 degree on the Path
            {
            ...
            if (JustEnter)
            {
                animal.transform.rotation *=
                    Quaternion.Slerp(Quaternion.identity, Target, OrientSmoothness * animal.DeltaTime * 2);
            }
